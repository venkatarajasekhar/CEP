 ...
 * Note(s):  	Alfred Lohmann
  ...
  * version V2.0
  *         adapted to the CEP-XYNERGY Board
  * date    2.05.2014
 *----------------------------------------------------------------------------
...
#define USARTxx                           USART6
#define USARTxx_CLK                       RCC_APB2Periph_USART6
#define USARTxx_CLK_INIT                  RCC_APB2PeriphClockCmd
#define USARTxx_IRQn                      USART6_IRQn
#define USARTxx_IRQHandler                USART6_IRQHandler

#define USARTxx_TX_PIN                    GPIO_Pin_6
#define USARTxx_TX_GPIO_PORT              GPIOC
#define USARTxx_TX_GPIO_CLK               RCC_AHB1Periph_GPIOC
#define USARTxx_TX_SOURCE                 GPIO_PinSource6
#define USARTxx_TX_AF                     GPIO_AF_USART6

#define USARTxx_RX_PIN                    GPIO_Pin_7
#define USARTxx_RX_GPIO_PORT              GPIOC
#define USARTxx_RX_GPIO_CLK               RCC_AHB1Periph_GPIOC
#define USARTxx_RX_SOURCE                 GPIO_PinSource7
#define USARTxx_RX_AF                     GPIO_AF_USART6
...
/*----------------------------------------------------------------------------
  USART6_IRQHandler
  Handles USART6 global interrupt request.
 *----------------------------------------------------------------------------*/
void USART6_IRQHandler (void) {
  ...
  struct buf_st *p;
    ...
    if (IIR & USART_SR_RXNE) {            /* read interrupt                   */
      USART6->SR &= ~USART_SR_RXNE;	      /* clear interrupt                  */

      p = &rbuf;

      if (((p->in - p->out) & ~(RBUF_SIZE-1)) == 0) {
        p->buf [p->in & (RBUF_SIZE-1)] = (USART6->DR & 0x1FF);
        p->in++;
      }
    }

    if (IIR & USART_SR_TXE) {
      USART6->SR &= ~USART_SR_TXE;	      /* clear interrupt                    */

      p = &tbuf;

      if (p->in != p->out) {
        USART6->DR = (p->buf [p->out & (TBUF_SIZE-1)] & 0x1FF);
        p->out++;
        tx_restart = 0;
      }
      else {
        tx_restart = 1;
        USART6->CR1 &= ~USART_SR_TXE;     /* disable TX IRQ if nothing to send  */
...
int USART_SendChar (int c) {
  ...
  if (tx_restart) {                       /* If TX interrupt is disabled        */
    ...
    USART6->CR1 |= USART_SR_TXE;          /* enable TX interrupt                */
  }
